# Team

Team resources, onboarding, and retrospectives.

Navigation ownership note: top-level documentation routing is owned by [`../CLAUDE.md`](../CLAUDE.md). Keep this file focused on team resources and update the root index first when paths move.

## Folders

| Folder | What's Here |
|--------|-------------|
| `onboarding-guides/` | Role-specific onboarding guides for new team members |
| `retros/` | Team retrospective notes |

## Onboarding Guides

| Guide | For |
|-------|-----|
| [onboarding-general.md](onboarding-guides/onboarding-general.md) | All new hires - shared tools, repos, channels, first tasks |
| [onboarding-product.md](onboarding-guides/onboarding-product.md) | Product managers |
| [onboarding-engineering.md](onboarding-guides/onboarding-engineering.md) | Engineers |
| [onboarding-design.md](onboarding-guides/onboarding-design.md) | Designers |
| [onboarding-analytics.md](onboarding-guides/onboarding-analytics.md) | Analysts |
| [onboarding-data-engineering.md](onboarding-guides/onboarding-data-engineering.md) | Data engineers |
